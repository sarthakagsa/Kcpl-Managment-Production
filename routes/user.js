...
router.post('/signup',async (req,res)=>{
    try {
        ...
        if (error.message == `E11000 duplicate key error collection: test.users index: email_1 dup key: { email: \"${req.body.email}\" }`) {
            return res.status(400).send({error : 'Email is aldready registered'})
        }
...
router.post('/users/logout/:token',auth,async (req,res)=>{
    try {
